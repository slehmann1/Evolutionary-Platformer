function [] = setupPlot(maxTime,level,generationNumber,iterationTime)
    ...
    %Have to check both conditions because of backwards compatibility, in older
    %versions matlab doesn't seem to return 0 when doing ishandle on a null
    %value
    if isempty(FIGUREWINDOW)|~ishandle(FIGUREWINDOW)
